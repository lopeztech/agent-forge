...
Recommended flow:
  ...
  6. bash to run the project's checks. On a fresh clone install dependencies
     first (e.g. \`npm ci\`) so dev tooling like \`tsc\` is present. If the
     project has a typecheck script (e.g. \`npm run typecheck\`), run it too — a
     test file can pass \`node --test\` at runtime but still fail
     \`tsc --noEmit\`, which CI gates on. Then run the test command. The
     finalize wrapper re-runs both as the gate, so iterate until both are green
     or it will bounce back to you.
