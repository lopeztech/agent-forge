...
Recommended flow:
  ...
  3. bash to run the project's checks yourself. On a fresh clone install
     dependencies first (e.g. \`npm ci\`) so the project's dev tooling — like
     \`tsc\` for typecheck — is actually present; then run typecheck first if
     the project has one (e.g. \`npm run typecheck\` — a change can pass tests
     but still fail \`tsc --noEmit\`, which CI gates on), then the test command (typically
